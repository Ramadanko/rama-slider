...
class AnimatedSlider extends FadeSlider {
  ...
  insertCssStyles(url: string): void {
    ...
    const cssStyle = `.overlay-number-${this.currentSlide}.${sliderId}
    .image-portion{background-image: url("${url}")}`;
    const style = document.createElement('style');
    style.innerHTML = cssStyle;
    document.body.appendChild(style);
  }

  setNextInterval(): void {
  ...
  takeSnapshot(): void {
    if (!this.savedSlides[this.currentSlide]) {
      domToImage.toBlob(this.trackContainer.children[this.currentSlide - 1]).then((blobImage: Blob) => {
  ...
  createOverlay(): void {
    ...
    newOverlay.className = `overlay ${this.currentAnimation} ${newContainer.getAttribute(
      'data-rama-id',
    )} overlay-number-${currentSlide}`;
